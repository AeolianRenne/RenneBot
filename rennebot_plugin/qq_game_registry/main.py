...
class Main(Star):
    ...
    async def route_message(self, event: AstrMessageEvent):
        ...
        try:
            ...
            elif sender_id in _configured_ids("AI_PRIVATE_USER_IDS") and message:
    ...
    async def _handle_group_message(
        ...
        if command.kind == CommandKind.AI:
            if group_id not in _configured_ids("AI_GROUP_IDS"):
                return "此群未启用 AI。"
    ...
    def _handle_registry_command(
        ...
        if command.kind == CommandKind.DELETE_GAME_ID:
            ...
            if target_user_id != sender_id and sender_id not in _configured_ids(
                "BOT_ADMIN_USER_IDS"
            ):
